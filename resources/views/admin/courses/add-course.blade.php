        ...
        <form action="{{route('course.store')}}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row col-lg-8 col-md-8 col-sm-8 col-xs-12" style="margin-left: 15%;">
                <div class="form-group">
                    <input name="name" type="text" class="form-control @error('name') is-invalid @enderror" placeholder="Course Name" value="{{ old('name')}}">
                    @error('name')
                    <div class="invalid-feedback" style="color: red;"> * {{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group">
                    <input name="duration" type="text" class="form-control @error('duration') is-invalid @enderror" placeholder="Course Duration (Number)" value="{{ old('duration')}}">
                    @error('duration')
                    <div class="invalid-feedback" style="color: red;"> * {{ $message }}</div>
                    @enderror
                </div>
